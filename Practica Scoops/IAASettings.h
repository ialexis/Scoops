//
//  IAASettings.h
//  Scoops
//
//  Created by Ivan on 21/04/15.
//  Copyright (c) 2015 Cloud On Mobile. All rights reserved.
//

#define AZUREMOBILESERVICE_ENDPOINT @"https://iaanewssolutionplatform.azure-mobile.net/"
#define AZUREMOBILESERVICE_APPKEY @"zQwCDsrkXcRPXJFcnhtJOlDMKtyKdu47"
#define NEWS_WRITED @"0"
#define NEWS_REVIEWED @"1"
#define NEWS_PUBLISED @"2"
#define TITLE_NEWS_WRITED @"Noticias Escritas"
#define TITLE_NEWS_REVIEWED @"Noticias Revisadas"
#define TITLE_NEWS_PUBLISED @"Noticias Publicadas"
#define SECTION_NEWS_WRITED 2
#define SECTION_NEWS_REVIEWED 1
#define SECTION_NEWS_PUBLISED 0
#define DID_NEW_NEWS_NOTIFICATION_NAME @"NEW_NEWS"


